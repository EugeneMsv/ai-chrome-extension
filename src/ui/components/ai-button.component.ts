// AiButton Component - UI Layer
// Main AI button that appears on text selection

import { SmartButton } from './smart-button.component';
import { BubbleButton } from './bubble-button.component';

let hideTimeout: NodeJS.Timeout | null = null; // Module-level timeout for hiding

export class AiButton extends SmartButton {
  public isShown: boolean = false;
  public bubblesPositioned: boolean = false; // Track if bubbles have been positioned
  public bubbleButtons: BubbleButton[] = [];
  public buttonContainer: HTMLDivElement | null;
  ...
  private _buildButtonContainer(positionX: number, positionY: number): HTMLDivElement {
  ...
  private _setupEventListeners(): void {
    ...
    this.onMouseOver = () => {
      ...
      this._onMouseOverDefault(); // This will change color and add shadow
    ...
    this.onMouseOut = () => {
      ...
      this._onMouseOutDefault(); // This will reset color and shadow
    ...
    if (this.buttonContainer) {
      ...
      this.buttonContainer.addEventListener('mouseout', (event: MouseEvent) => {
        // Start hide timer only if the mouse truly left the container and its children
        if (!this.buttonContainer || !this.buttonContainer.contains(event.relatedTarget as Node)) {
          this.hideWithTimeout();
        }
      });

      // Prevent mouseup on the container from triggering document listener
      this.buttonContainer.addEventListener('mouseup', (event: Event) => {
        event.stopPropagation();
      });
    }
  }

  public addBubbleButtons(buttons: BubbleButton[]): void {
    this.bubbleButtons = buttons;
    // Don't position them immediately, wait for hover
    this.bubblesPositioned = false;
  }

  private _positionBubbleButtons(): void {
